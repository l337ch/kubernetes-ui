  ...
  angular.module('kubernetesApp.services')
    .service('cAdvisorService', function($http, $q, ENV){
      ...
      function getContainerInfo(minionIp, containerId){
        ...
        $http.post(fullUrl, request).success(function(data){
          deferred.resolve(data);
        }).error(function(){
          deferred.reject('There was an error')
        })
      ...
      this.getDataForMinion = function(minionIp){
        ...
        var p = $q.all([getMachineInfo(minionIp), getContainerInfo(minionIp)]).then(function(dataArray){
          ...
          deferred.resolve({memoryData: memoryData, cpuData: cpuData, filesystemData: fsData, machineData: machineData, containerData: containerData});
      ...
      function parseCpu(machineInfo, containerInfo) {
        ...
        return {cpuPercentUsage: cpuUsage};
      }
